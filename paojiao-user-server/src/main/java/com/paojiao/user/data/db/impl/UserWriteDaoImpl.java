...
public class UserWriteDaoImpl implements IUserWriteDao {
	...
	@Override
	@Caching(evict = {@CacheEvict(key = RedisKeyUtil.USER_INFO, value = RedisKeyUtil.CACHE_NAME),
			@CacheEvict(key = RedisKeyUtil.USER_DB_INFO, value = RedisKeyUtil.CACHE_NAME)})
	...
	@Caching(evict = {@CacheEvict(key = RedisKeyUtil.USER_INFO, value = RedisKeyUtil.CACHE_NAME),
			@CacheEvict(key = RedisKeyUtil.USER_DB_ATTR_INFO, value = RedisKeyUtil.CACHE_NAME)})
	@Override
